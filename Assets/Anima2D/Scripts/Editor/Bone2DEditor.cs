...
namespace Anima2D
{
    ...
    public class Bone2DEditor : Editor
    {
        ...
        {
            if (Selection.transforms.Length == 1 && !m_ChildTransformProperty.objectReferenceValue &&
                m_ChildProperty.objectReferenceValue)
            {
        ...
        void OnSceneGUI()
        {
            if (Tools.current == Tool.Move)
            {
                ...
                {
                    UnityEngine.Camera.current.transform.rotation = m_Bone.transform.rotation;
                }
